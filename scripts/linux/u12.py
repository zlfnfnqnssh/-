...
# -*- coding: utf-8 -*-
"""
U-12 /etc/services 파일 소유자 및 권한 설정 점검 스크립트
- collected_value에 실제 ls -l /etc/services 결과 한 줄이 그대로 들어감
"""
...
OUTPUT_FILENAME_TEMPLATE = os.getenv("OUTPUT_FILENAME", "u12_result_{scan_id}.json")
...
def save_json(result: Dict, output_dir: str, filename: str):
    ...
    try:
        ...
        print(f"[+] U-12 결과가 저장되었습니다: {filepath}")
    except Exception as e:
        print(f"[-] 파일 저장 실패: {e}")
...
def main():
    ...
    # ==================== /etc/services 파일 권한 및 소유자 확인 ====================
    ls_result = run_shell("ls -l /etc/services")

    # collected_value에 실제 ls -l 출력의 첫 번째 줄 그대로 저장
    if ls_result:
        collected_value = ls_result.splitlines()[0].strip()
    else:
        collected_value = "파일 없음 또는 접근 불가 (/etc/services 파일이 존재하지 않음)"

    check_results.append({
        "sub_check": "/etc/services 파일 권한 및 소유자",
        "config_file": "/etc/services",
        "collected_value": collected_value,        # ← 실제 ls -l 한 줄 그대로
        "raw_output": ls_result,
        "service_status": "N/A",
        "source_command": "ls -l /etc/services"
    })
