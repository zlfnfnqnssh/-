...
# -*- coding: utf-8 -*-
"""
U-07 /etc/passwd 파일 소유자 및 권한 설정 점검 스크립트
...
OUTPUT_FILENAME_TEMPLATE = os.getenv("OUTPUT_FILENAME", "u07_result_{scan_id}.json")
...
def save_json(result: Dict, output_dir: str, filename: str):
    ...
    try:
        ...
        print(f"[+] U-07 결과가 저장되었습니다: {filepath}")
    except Exception as e:
        print(f"[-] 파일 저장 실패: {e}")
...
def main():
    ...
    # ==================== /etc/passwd 파일 권한 및 소유자 확인 ====================
    # ls -l 명령어로 실제 출력 그대로 수집
    ls_result = run_shell("ls -l /etc/passwd")

    # collected_value에 실제 한 줄 그대로 저장 (예: -rw-r--r-- 1 root root ...)
    if ls_result:
        # 첫 번째 줄만 사용 (보통 한 줄 나옴)
        collected_value = ls_result.splitlines()[0].strip()
    else:
        collected_value = "파일 확인 실패"

    check_results.append({
        "sub_check": "/etc/passwd 권한 및 소유자",
        "config_file": "/etc/passwd",
        "collected_value": collected_value,        # ← 실제 ls -l 한 줄 그대로
        "raw_output": ls_result,
        "service_status": "N/A",
        "source_command": "ls -l /etc/passwd"
    })
