...
OUTPUT_FILENAME_TEMPLATE = os.getenv("OUTPUT_FILENAME", "u35_result_{scan_id}.json")
...
def save_json(result: Dict, output_dir: str, filename: str):
    try:
        ...
        print(f"[+] U-35 결과가 저장되었습니다: {filepath}")
    except Exception as e:
        print(f"[-] 파일 저장 실패: {e}")
...
def main():
    ...
    for conf_path in apache_conf_paths:
        if os.path.exists(conf_path):
            ...
            # 실제 grep 명령어 (collected_value 추출용)
            grep_cmd = f"grep -E 'Options' {conf_path}"

            # 정규표현식으로 Options 지시자 내 Indexes 관련 핵심 추출
            options_lines = re.findall(r'^\s*Options\s+.*$', conf_raw, re.MULTILINE | re.IGNORECASE)
            indexes_parts = []
            for line in options_lines:
                if re.search(r'(?<!\w)([-+]?Indexes)(?!\w)', line, re.IGNORECASE):
                    indexes_parts.append(line.strip())

            collected_value = "\n".join(indexes_parts) if indexes_parts else "Indexes 옵션 없음"

            check_results.append({
                "sub_check": "Apache Directory Listing (Indexes)",
