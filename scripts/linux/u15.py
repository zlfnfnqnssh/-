...
# -*- coding: utf-8 -*-
"""
U-15 world writable 파일 점검 스크립트
- world writable 파일 (-perm -2) 검색
...
OUTPUT_FILENAME_TEMPLATE = os.getenv("OUTPUT_FILENAME", "u15_result_{scan_id}.json")
...
def save_json(result: Dict, output_dir: str, filename: str):
    ...
    try:
        ...
        print(f"[+] U-15 결과가 저장되었습니다: {filepath}")
    except Exception as e:
        print(f"[-] 파일 저장 실패: {e}")
...
def main():
    ...
    # 주통기 가이드 기반: find / -type f -perm -2
    find_cmd = 'find / -type f -perm -2 -xdev -ls 2>/dev/null | head -100'

    result_raw = run_shell(find_cmd)

    if "TIMEOUT" in result_raw:
        collected_value = "검색 시간 초과"
        raw_output = "find 명령어가 제한 시간(90초)을 초과하여 중단되었습니다."
        ...
        collected_value = "없음"
        raw_output = "world writable 파일이 없습니다."

    check_results.append({
        "sub_check": "World Writable 파일 검색",
