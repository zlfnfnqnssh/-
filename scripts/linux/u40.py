...
# -*- coding: utf-8 -*-
"""
U-40 웹서비스 파일 업로드 및 다운로드 제한 점검 스크립트
- LimitRequestBody 지시자 확인
...
OUTPUT_FILENAME_TEMPLATE = os.getenv("OUTPUT_FILENAME", "u40_result_{scan_id}.json")
...
def save_json(result: Dict, output_dir: str, filename: str):
    try:
        ...
        print(f"[+] U-40 결과가 저장되었습니다: {filepath}")
    except Exception as e:
        print(f"[-] 파일 저장 실패: {e}")
...
def main():
    ...
    for conf_path in apache_conf_paths:
        if os.path.exists(conf_path):
            ...
            grep_cmd = f"grep -i 'LimitRequestBody' {conf_path}"

            # grep 실행 결과
            limit_raw = run_shell(grep_cmd)

            # 정규표현식으로 LimitRequestBody 지시자만 정확히 추출
            limit_lines = re.findall(
                r'^\s*LimitRequestBody\s+\d+', 
                conf_raw, 
                re.MULTILINE | re.IGNORECASE
            )

            collected_value = "\n".join([line.strip() for line in limit_lines]) if limit_lines else "LimitRequestBody 설정 없음"

            check_results.append({
                "sub_check": "Apache LimitRequestBody (파일 업로드/다운로드 제한)",
    ...
    if not found:
        check_results.append({
            ...
            "service_status": apache_svc,
            "source_command": "grep -i 'LimitRequestBody' /etc/apache2/apache2.conf"
        })
        ...
        "items": [
            {
                "category": "서비스 관리",
                "item_code": "U-40",
                "item_name": "웹서비스 파일 업로드 및 다운로드 제한",
