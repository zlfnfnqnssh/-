...
# -*- coding: utf-8 -*-
"""
U-02 패스워드 복잡성 설정 점검 스크립트
- Linux (Debian/Ubuntu + RHEL 계열), Solaris, AIX, HP-UX 모두 지원
...
OUTPUT_FILENAME_TEMPLATE = os.getenv("OUTPUT_FILENAME", "u02_result_{scan_id}.json")
...
def save_json(result: Dict, output_dir: str, filename: str):
    ...
    try:
        ...
        print(f"[+] U-02 결과가 저장되었습니다: {filepath}")
        return True
    except Exception as e:
        print(f"[-] 파일 저장 실패: {e}")
        return False
...
def main():
    ...
    os_checks: Dict[str, List[tuple]] = {
        "linux": [
            # Debian/Ubuntu 계열
            ("common-password (Debian/Ubuntu)", "/etc/pam.d/common-password", "pam_pwquality|pam_cracklib|minlen|lcredit|ucredit|dcredit|ocredit", "passwd"),
            ("pwquality.conf", "/etc/security/pwquality.conf", "minlen|lcredit|ucredit|dcredit|ocredit|minclass|difok", "passwd"),
            ("login.defs", "/etc/login.defs", "PASS_MIN_LEN|PASS_MIN_DAYS|PASS_MAX_DAYS", "passwd"),
            
            # RHEL/CentOS 계열 (참고용으로 유지)
            ("system-auth (RHEL)", "/etc/pam.d/system-auth", "pam_pwquality|pam_cracklib", "passwd"),
            ("pwquality.conf (RHEL)", "/etc/security/pwquality.conf", "minlen|lcredit|ucredit|dcredit|ocredit", "passwd"),
        ],
        "solaris": [
            ("default/passwd", "/etc/default/passwd", "PASSLENGTH|MINALPHA|MINNONALPHA|MINUPPER|MINLOWER|MINDIGIT|MINSPECIAL|NAMECHECK", "passwd"),
        ],
        "aix": [
            ("security/user", "/etc/security/user", "minlen|minalpha|minother|mindiff|maxrepeats|dictionlist", "passwd"),
        ],
        "hpux": [
            ("default/security", "/etc/default/security", "MIN_PASSWORD_LENGTH|PASSWORD_MIN_UPPER_CASE_CHARS|PASSWORD_MIN_LOWER_CASE_CHARS|PASSWORD_MIN_DIGIT_CHARS|PASSWORD_MIN_SPECIAL_CHARS", "passwd"),
        ]
    }
    ...
    for sub_check, config_file, keyword, svc in checks:
        # 서비스 상태 판단
        if target_os == "linux":
            # passwd 명령어 존재 여부로 기본 판단
            installed = "INSTALLED" if run_shell("command -v passwd >/dev/null 2>&1 && echo 1 || echo 0") else "NOT_INSTALLED"
