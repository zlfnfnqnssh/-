...
def check():
    category = "보안 관리"
    item_code = "W-14"
    item_name = "원격터미널 접속 가능한 사용자 그룹 제한"
    
    cmd_str = r'''$deny = (Get-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server' -Name fDenyTSConnections -ErrorAction SilentlyContinue).fDenyTSConnections; Write-Output "fDenyTSConnections = $deny"; net localgroup "Remote Desktop Users" 2>$null'''
    ...
    try:
        ...
        # 규칙 기반 평가는 아직 보류하고, 철저하게 모든 명령어의 결과값을 수집하는 완벽한 템플릿
        # LLM이 데이터 원문을 보고 종합 판정할 수 있도록 '규칙불가' 타입 유지 후 데이터만 공급
        output = {
            "category": category,
            "item_code": item_code,
            "item_name": item_name,
