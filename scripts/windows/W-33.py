...
def check():
    category = "서비스 관리"
    item_code = "W-33"
    item_name = "HTTP/FTP/SMTP 배너 차단"
    
    cmd_str = r'''if (Get-Command Get-WebConfigurationProperty -ErrorAction SilentlyContinue) { Get-WebConfigurationProperty -Filter system.webServer/httpProtocol/customHeaders -Name . -ErrorAction SilentlyContinue } else { 'N/A: IIS 미설치 - 해당 점검 항목 불필요' }'''
