...
def check():
    ...
    cmd_str = r'''$v = (Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon' -Name AllocateDASD -ErrorAction SilentlyContinue).AllocateDASD; if ($null -ne $v) { "AllocateDASD = $v" } else { 'AllocateDASD 속성 미존재 (기본값 0 = Administrators만 이동식 미디어 포맷 허용)' }'''
    ...
    try:
        ...
        result = "규칙불가"
        if full_out:
            if "= 0" in full_out or "미존재" in full_out or "미설정" in full_out:
                result = "양호"
            elif "= 1" in full_out or "= 2" in full_out:
                result = "취약"
