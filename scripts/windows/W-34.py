...
def check():
    ...
    cmd_str = r'''Get-Service TlntSvr -ErrorAction SilentlyContinue | Select-Object Name,Status,StartType; Get-WindowsFeature Telnet-Server -ErrorAction SilentlyContinue | Select-Object Name,InstallState'''
    ...
    try:
        ...
        result = "규칙불가"
        if full_out:
            if "Running" in full_out:
                result = "취약"
            elif "Stopped" in full_out or "not found" in full_out.lower() or "없" in full_out:
                result = "양호"
            else:
                result = "양호"
        else:
