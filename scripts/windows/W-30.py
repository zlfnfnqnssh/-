...
def check():
    ...
    cmd_str = r'''Get-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\SNMP\Parameters\ValidCommunities' -ErrorAction SilentlyContinue | Format-List'''
    ...
    try:
        ...
        if full_out is not None:
            ...
            if not full_out.strip() or "속성" not in lower_out and ":" not in lower_out and "=" not in lower_out:
                result = "양호"
            elif "public" in lower_out or "private" in lower_out:
                result = "취약"
            else:
                result = "양호"
