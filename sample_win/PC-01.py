...
def check():
    ...
    cmd_str = r'''net accounts; Get-LocalUser | Select-Object Name,Enabled,PasswordLastSet,PasswordExpires | Format-Table -AutoSize'''
    ...
    try:
        ...
        # 규칙 기반 판정: 최대 암호 사용 기간 <= 90일
        result_val = "규칙불가"
        try:
            match = re.search(r'Maximum password age[^\d]*(\d+|Unlimited)', full_out, re.IGNORECASE)
            if not match:
                match = re.search(r'최대 암호 사용 기간[^\d]*(\d+)', full_out)
            if match:
