...
`integration/` 디렉토리의 어댑터 4개가 PostgreSQL `forensic_db`를 허브로 모든 모듈의 흐름을 연결합니다. 각 모듈은 자기 영역 코드 변경 거의 0줄로 통합됨.

| 모듈 | 영역 | 코드 위치 |
|---|---|---|
| **PDF 가이드라인** | 입력·비교·변경 이력 | `tools/jutonggi_parser/` (PDF 파서 + sync_items)<br>`tools/mcp_server/` (MCP 인터페이스) |
| **Linux 점검 엔진** | Linux 자체 파이프라인 | `vulnerability-scanner/scripts/linux/U-01~U-72.py`<br>`tools/syeon_engine/` (runner/collector/batch_judge/main 등 8파일) |
| **Windows + 백엔드** | Windows 점검 + 패치 + DB | `vulnerability-scanner/scripts/{windows,pc}/`<br>`vulnerability-scanner/{engine,database,web/routes/scan,patch}` |
| **웹 UI + 운영** | 사용자 관리·감사 화면 | `vulnerability-scanner/web/templates/*` (Tailwind)<br>`web/routes/admin.py` |

데이터 흐름: PDF → native 테이블(vulnerabilities/history/changelog) → `vs_guideline_items` → SQLite 미러(Linux 엔진용) / LLM 판정 → `vs_judgments` → UI 표시 → 패치 실행 → `vs_patch_executions`. 자세한 통합 내역은 [INTEGRATION_FIXES.md](INTEGRATION_FIXES.md) 참조.
...
- 새 KISA 가이드라인 PDF 업로드 → 파서로 파싱 → forensic_db 적재
- **PDF 한 명령으로 4단계 자동**: vulnerabilities (native) → vulnerabilities_history → item_changelog → vs_guideline_items → SQLite guidelines.db
...
- Linux 일 때는 `tools.syeon_engine.main.run_pipeline` 으로 위임 (`integration/syeon_db_adapter.py`가 결과를 PostgreSQL로 통합)
...
| RDB | PostgreSQL (Docker, DB: `forensic_db`, `vs_*` 11개 테이블 + native 3개) |
| SQLite | Linux 엔진 batch_judge 가이드라인 미러 (`tools/syeon_engine/db/guidelines.db`) |
| 웹 프레임워크 | FastAPI + Jinja2 (SSR) |
| ASGI 서버 | uvicorn (포트 8081) |
| UI | Tailwind CSS |
| PDF 파싱 | pdfplumber (PDF 어댑터) / PyMuPDF |
...
│   └── reference/2021_*.pdf
├── tools/                        # 가이드라인 + Linux 엔진 모듈
│   ├── jutonggi_parser/          # PDF 파서 + DB sync (forensic_db.vulnerabilities)
│   ├── mcp_server/               # MCP 서버
│   └── syeon_engine/             # Linux 파이프라인 (8 .py)
└── vulnerability-scanner/        # Windows/백엔드 + 웹 UI 모듈
    ├── main.py                   # FastAPI 앱 (포트 8081)
    ├── .env / .env.example
    ├── requirements.txt
    ├── config/                   # settings + machine_id
    ├── auth/                     # 세션 기반 인증
    ├── integration/              # 통합 어댑터 (4개)
    │   ├── euni_adapter.py       #   PDF 파서 → forensic_db (vulnerabilities + vs_guideline_items + SQLite ETL)
    │   ├── syeon_db_adapter.py   #   Linux 엔진 DBWriter 호환 PostgreSQL 어댑터
...
### native 3개 테이블 (`forensic_db` 안에 공존)
| 테이블 | 역할 |
|---|---|
| `vulnerabilities` | PDF 파서 마스터 (현재 가이드라인) |
| `vulnerabilities_history` | PDF 버전 이력 |
| `item_changelog` | 항목별 변경 로그 (added/modified/removed) |

MCP 서버는 native 테이블을, LLM 판정은 `vs_*`을 사용. 어댑터가 한 방향 sync.

---

## 향후 확장 과제

- ~~Linux 흐름 통합~~ — 완료, WSL 환경 검증 남음
