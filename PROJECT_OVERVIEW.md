...
━━━━━━━━━━━━━━━━━━━━━━
  ...
  대상 시스템(Linux or Windows) ──→ 수집 스크립트 실행 ──→ 결과 정규화(JSON) ──→ RDB 저장

  * Linux: Python + subprocess (계정, 파일권한, 서비스, 네트워크, 설정파일)
  * Windows: Python + PowerShell (계정, 레지스트리, 서비스, 방화벽, 감사정책)
  * 수집 결과를 통일된 JSON 스키마로 정규화
  * SQLite(개발) / PostgreSQL(운영)에 저장


[3단계] RAG 검색 + LLM 판정 (핵심) — 병렬 처리
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  ※ 항목이 수십~수백 개이므로, 순차 처리 시 너무 느림
  ※ asyncio 기반 병렬 처리로 여러 항목을 동시에 판정

  DB에서 미판정 항목 N개를 배치로 조회
       │
       ▼
  ┌─────────────────── 병렬 처리 (asyncio) ───────────────────┐
  │                                                           │
  │  항목1: 임베딩 → VDB검색 → LLM추론 → DB저장               │
  │  항목2: 임베딩 → VDB검색 → LLM추론 → DB저장  (동시 실행)   │
  │  항목3: 임베딩 → VDB검색 → LLM추론 → DB저장  (동시 실행)   │
  │  ...                                                      │
  │  항목N: 임베딩 → VDB검색 → LLM추론 → DB저장  (동시 실행)   │
  │                                                           │
  └───────────────────────────────────────────────────────────┘
       │
       ▼
  ...
  * 각 워커는 독립적: 임베딩 → VDB검색 → LLM호출 → 결과저장
